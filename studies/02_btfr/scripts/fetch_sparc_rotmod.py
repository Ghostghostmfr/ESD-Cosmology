"""Fetch and cache the full SPARC catalog: master MRT table + Rotmod_LTG/.

Mirrors used (CWRU SPARC distribution):
  http://astroweb.cwru.edu/SPARC/SPARC_Lelli2016c.mrt   (~32 KB, 175 galaxies)
  http://astroweb.cwru.edu/SPARC/Rotmod_LTG.zip          (~600 KB zipped)

On success writes:
  data/SPARC_Lelli2016c.mrt
  data/Rotmod_LTG/<Galaxy>_rotmod.dat   (one per galaxy, 175 files)

Idempotent: skips downloads if the cache already exists.  Use
`--force` to redownload.
"""
...
# Local workspace mirror populated by the legacy supporting-paper pipeline.
# Checked first; if present we copy from it and skip the network entirely.
...
MRT_URLS = [
    ...
    # VizieR fallback (different format; not preferred)
    ...
    for url in urls:
        ...
        try:
            req = urllib.request.Request(
                url, headers={"User-Agent": "esd-btfr/1.0"}
            )
    ...
    for url in urls:
        ...
        try:
            ...
            with zipfile.ZipFile(io.BytesIO(payload)) as zf:
                n_before = len(os.listdir(extract_to))
                # SPARC tarball stores files at the archive root; flatten any
                # leading directory the zip may use.
                for info in zf.infolist():
                    ...
                    with open(os.path.join(extract_to, name), "wb") as dst:
                        dst.write(data)
                n_after = len(os.listdir(extract_to))
            print(
                f"[fetch]   -> extracted {n_after - n_before} new files to "
                f"{extract_to} (total {n_after})"
            )
            return
        except Exception as e:  # noqa: BLE001
            print(f"[fetch]   -> failed: {e}")
            last_err = e
    raise RuntimeError(
        f"All mirrors failed for rotmod archive; last error: {last_err}"
    )


def _copy_from_local(force: bool) -> tuple[bool, bool]:
    """Try to populate caches from the local workspace mirror.
    Returns (have_mrt, have_rotmod) after the copy attempt."""
...
def fetch(force: bool = False) -> None:
    ...
    if not have_rotmod:
        ...
        n = len(
            [f for f in os.listdir(ROTMOD_DIR) if f.endswith("_rotmod.dat")]
        )
